...
def generate_audit_pdf(audit_data: Dict) -> Optional[bytes]:
    ...
    contract_hash = audit_data.get("contract_hash", "")
    ...
    # Report ID + Contract Hash
    if report_id:
        elements.append(Paragraph(
            f'Report ID: <b>{report_id}</b> &nbsp;|&nbsp; '
            f'Contract Hash: <font face="Courier" size="8">{contract_hash[:16]}...{contract_hash[-8:]}</font>',
            ParagraphStyle('ReportMeta', parent=styles['Normal'],
                           fontSize=8, textColor=HexColor("#666666"),
                           spaceAfter=4)
        ))
        elements.append(Paragraph(
            f'Verify: <font color="#0099cc">auditsmart.org/verify/{report_id}</font>',
            ParagraphStyle('VerifyLink', parent=styles['Normal'],
                           fontSize=8, textColor=HexColor("#0099cc"),
                           spaceAfter=8)
        ))
    ...
    disclaimer_data = [[Paragraph(
        '<font color="#cc6600"><b>DISCLAIMER:</b></font> '
        'This is an AI-generated security assessment, NOT a certified security audit. '
        'AuditSmart provides automated vulnerability scanning as a preliminary screening tool. '
        ...
        'By using AuditSmart, you acknowledge that Xorion Network LLC bears no responsibility '
        'for exploits, hacks, or losses related to scanned contracts.',
        ParagraphStyle('DisclaimerText', parent=styles['Normal'],
                       fontSize=7, textColor=HexColor("#996633"),
                       leading=10)
    )]]
    disclaimer_table = Table(disclaimer_data, colWidths=[480])
    disclaimer_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, -1), HexColor("#fff8f0")),
        ('BOX', (0, 0), (-1, -1), 0.5, HexColor("#ffcc88")),
        ('PADDING', (0, 0), (-1, -1), 8),
    ]))
    ...
    elements.append(Paragraph(
        "LEGAL DISCLAIMER: This AI Security Assessment Report is generated by AuditSmart, "
        "a product of Xorion Network LLC. This is NOT a certified security audit. "
        "AuditSmart uses multi-agent AI to identify potential vulnerabilities as a preliminary "
        "screening tool. No automated tool can guarantee 100% vulnerability coverage. "
        "Xorion Network LLC and AuditSmart accept no liability for any losses, damages, or "
        "exploits arising from vulnerabilities not detected by this scan. "
        "For production contracts handling significant value, a professional manual audit "
        "by certified security researchers is strongly recommended.",
    ...
    elements.append(Paragraph(
        ...
        f"— auditsmart.org — © {datetime.utcnow().year} Xorion Network LLC",
        styles['Footer']
    ))
